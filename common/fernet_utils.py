...
class FernetUtils(object):
    ...
    def validate_key_repository(self, requires_write=False):
        ...
        if not is_valid:
            app_log.error('Either [%s] key_repository does not exist '
                    'or Keystone does not have sufficient permission to '
                    'access it: %s',   self.config_group, self.key_repository)
        else:
            ...
            if(stat_info.st_mode & stat.S_IROTH or
               stat_info.st_mode & stat.S_IXOTH):
                app_log.warning('key_repository is world readable: %s', self.key_repository)
    ...
    def create_key_directory(self, keystone_user_id=None,
        ...
        if not os.access(self.key_repository, os.F_OK):
            app_log.info('key_repository does not appear to exist; attempting to create it')

            try:
                os.makedirs(self.key_repository, 0o700)
            except OSError:
                app_log.error('Failed to create key_repository: either it already '
            ...
            if keystone_user_id and keystone_group_id:
                ...
                app_log.warning('Unable to change the ownership of key_repository without '
    ...
    def _create_tmp_new_key(self, keystone_user_id, keystone_group_id):
        ...
        if keystone_user_id and keystone_group_id:
            ...
            app_log.warning('Unable to change the ownership of the new key without a '
        ...
        try:
            ...
            app_log.error('Failed to create new temporary key: %s', key_file)
            raise
        finally:
        ...
        app_log.info('Created a new temporary key: %s', key_file)
    ...
    def _become_valid_new_key(self):
        ...
        app_log.info('Become a valid new key: %s', valid_key_file)
    ...
    def initialize_key_repository(self, keystone_user_id=None,
        ...
        if os.access(os.path.join(self.key_repository, '0'),
                     os.F_OK):
            app_log.info('Key repository is already initialized; aborting.')
            return
    ...
    def rotate_keys(self, keystone_user_id=None, keystone_group_id=None):
        ...
        app_log.info('Starting key rotation with %s key files: %s', len(key_files), list(key_files.values()))
        ...
        app_log.info('Current primary key is: %s', current_primary_key)
        new_primary_key = current_primary_key + 1
        app_log.info('Next primary key will be: %s', new_primary_key)
        ...
        key_files[new_primary_key] = os.path.join(
            self.key_repository,
            str(new_primary_key))
        app_log.info('Promoted key 0 to be the primary: %s', new_primary_key)
        ...
        while len(keys) > (max_active_keys - 1):
            ...
            app_log.info('Excess key to purge: %s', key_to_purge)
    ...
    def load_keys(self, use_null_key=False):
        ...
        if not self.validate_key_repository():
            ...
            self.initialize_key_repository()
