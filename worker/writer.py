...
if Image is None:
    ...
    class ImageWriter(BaseWriter):
        ...
        def _paint_text(self, xpos, ypos):
            global _FONT
            if not _FONT:
                _FONT = ImageFont.truetype(FONT, self.font_size*5)  #原来是*2
            width, height = _FONT.getsize(self.text)
            #pos = (mm2px(xpos, self.dpi) - width // 2,
            #       mm2px(ypos, self.dpi) - height // 4)
            pos = (mm2px(xpos, self.dpi) - width // 2+40,
                   mm2px(ypos, self.dpi)- height)
            self._draw.text(pos, self.text, font=_FONT, fill=self.foreground)
